...
ACP supports [MCP servers](/docs/mcp) defined in a project-level or user-level `.cursor/mcp.json`. Launch `agent` from your project directory and approve the servers you want to use.
...
MethodTypeUse`cursor/ask_question`BlockingAsk users multiple-choice questions`cursor/create_plan`BlockingRequest explicit plan approval`cursor/update_todos`NotificationNotify client about todo state updates`cursor/task`NotificationNotify client about subagent task completion`cursor/generate_image`NotificationNotify client about generated image output
### cursor/ask_question
...
- **JetBrains IDEs** — Connect IntelliJ IDEA, WebStorm, PyCharm, or other JetBrains IDEs to Cursor's agent. See the [JetBrains integration guide](/docs/integrations/jetbrains) for setup instructions.
...
- [MCP in CLI](/docs/cli/mcp): Manage and use MCP servers from Cursor CLI
- [MCP Overview](/docs/mcp): Learn MCP transports, configuration, and server setup